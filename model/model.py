...
class OneVideoProcessor:
    def __init__(self, cb_model=None, emb_model=None, torch_path=None, transformer=None, device=device):
        if transformer is None:
            print('Не указан трансформер')
            return

        self.transformer = MultimodalTransformer(device=device)
        ...
        torch_model.load_state_dict(torch.load(torch_path, weights_only=True))
