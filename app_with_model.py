...
import time
import torch
from model import MultimodalTransformer, MultimodalInterviewModel, OneVideoProcessor
...
# Инициализация процессора видео с моделями и трансформером
processor = OneVideoProcessor(
    torch_path='path/to/torch_model_weights.pth',  # Укажите путь к весам модели
...
def predict_ocean_from_video(video_file, transcription):
    """
    Функция для предсказания черт OCEAN на основе видеофайла и транскрипции.
    Использует обученную модель из файла `model.py`.

    Args:
        video_file (str): Путь к видеофайлу.
        transcription (str): Транскрипция речи из видео.

    Returns:
        dict: Словарь с предсказанными значениями OCEAN и 'interview'.
    """

    # Предсказание с помощью нейросетевой модели PyTorch
    pred_dict = processor.predict_pt(video_file, transcription)
    
    # Преобразуем значения в диапазон от 0 до 100, как в заглушке, для совместимости
    ocean_scores = {trait: round(float(score.item()) * 100) for trait, score in pred_dict.items()}
...
# Функция для поиска похожих вакансий
def find_similar_vacancies(video_description):
    vacancies = session.query(Vacancy).all()
    vacancy_descriptions = [vacancy.description for vacancy in vacancies]

    tokenized_descriptions = [desc.lower().split() for desc in vacancy_descriptions]
    
    bm25 = BM25Okapi(tokenized_descriptions)
    
    tokenized_query = video_description.lower().split()
    scores = bm25.get_scores(tokenized_query) 
    
    ranked_vacancies = sorted(zip(vacancies, scores), key=lambda x: x[1], reverse=True)
        ...
        with col1:
            if st.button("Зарегистрироваться"):
                ...
                if existing_user:
                    ...
                    st.error("Электронная почта уже зарегистрирована") 
                else:
                    new_user = User(username=username, password=password, email=email, user_type='user')
                    session.add(new_user)
                    session.commit()
...
def user_dashboard():
    ...
    if videos:
        ...
        similar_vacancies = find_similar_vacancies(last_video.description) 
        
        st.subheader("Похожие вакансии:")
        for idx, (vacancy, score) in enumerate(similar_vacancies[:5], 1):
            st.write(f"{idx}. {vacancy.title} - Релевантность: {score:.2f}")
    ...
    if 'upload_new_video' in st.session_state and st.session_state['upload_new_video']:
        ...
        if video_file is not None:
            ...
            similar_vacancies = find_similar_vacancies(transcribed_text)  
...
def admin_dashboard():
    ...
    if st.session_state['admin_page'] == 'main':
        ...
        if vacancy:
            ...
            #     candidate_videos = session.query(Video).all()
            #     if candidate_videos:
            #         ranked_candidates = sorted(candidate_videos, key=lambda x: x.interview_score, reverse=True)
            #         st.subheader("Ранжированные кандидаты с контактами:")
            #         for idx, candidate in enumerate(ranked_candidates, 1):
            #             candidate_user = session.query(User).filter_by(id=candidate.user_id).first()
            #             if candidate_user:
            #                 st.write(f"{idx}. Вероятность интервью: {candidate.interview_score}%")
            #                 st.write(f"Имя пользователя: {candidate_user.username}")
            #                 st.write(f"Контакт (email): {candidate_user.email}")
            #                 st.video(f"uploaded_videos/{candidate_user.username}/{candidate.filename}")
            #                 st.write("---")
            if st.button("Получить рейтинг кандидатов"):
